...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):#path待预测的图片路径
        ...
        if '*' in p:
            ...
            files = [p]  # 得到files，所有需要测试的图片的路径
        else:
            raise Exception(f'ERROR: {p} does not exist')

        images = [x for x in files if x.split('.')[-1].lower() in img_formats]#对输入进行过滤，图片放在images
        videos = [x for x in files if x.split('.')[-1].lower() in vid_formats]#视频放在videos里面
        ni, nv = len(images), len(videos)#得到带预测的图片文件数量和视频数量

        self.img_size = img_size
        self.stride = stride
        self.files = images + videos#所有视频和图片的路径
        self.nf = ni + nv  # number of files文件数量
    ...
    def __iter__(self):
        self.count = 0#处理到第几个文件了
        return self

    def __next__(self):
        if self.count == self.nf:#处理文件数是否等于总文件数
            raise StopIteration#停止迭代
        path = self.files[self.count]#斗则继续
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img0 = cv2.imread(path)  # BGR加载图片
            assert img0 is not None, 'Image Not Found ' + path#为空则为异常


        # Padded resize
        img = letterbox(img0, self.img_size, stride=self.stride)[0]#处理图片长边变成640

        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416把通道放在最前面
        img = np.ascontiguousarray(img)#使地址连续

        return path, img, img0, self.cap#path文件路径，img处理后的图片数据，img0是处理前图片数据
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Arguments
        ...
        weights:        Train, val, val weights (list)
