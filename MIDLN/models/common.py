...
from utils.keshihua import feature_visualization
from models.SPACEAttentionModel import SPACEAttentionModel, Attention, AttentionLePE
...
class PFeh(nn.Module):######### MFEHead ############
    def __init__(self, in_ch, out_ch, act=True):
        super(PFeh, self).__init__()
    ...
    def forward(self, input):
        ...
        out = self.depth_conv(out)
        # out = self.depth_conv(out)
        out1 = self.point_conv(out)
        return out1
class Convatt1(nn.Module):# ##############SWAM######################
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Convatt1, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.att = SPACEAttentionModel(c2)
...
class SPPCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cv7(torch.cat((y1, y2), dim=1))#self.att(self.cv7(torch.cat((y1, y2), dim=1)))
