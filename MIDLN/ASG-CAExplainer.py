...
#######E:\anaconda3\envspytorch1.11.0\Lib\site-packages\pytorch_grad_cam\utils\image.py二值化关键
    ...
    def __call__(self, img_path, save_path,):
        ...
        for i in trange(int(post_result.size(0) * self.ratio)):
            ...
            if weights.shape ==(1, 128):
                max_indices = np.argpartition(weights, -12, axis=None)[-12:]
            elif weights.shape == (1, 64):
                max_indices = np.argpartition(weights, -8, axis=None)[-8:]
            elif weights.shape == (1, 256):
                max_indices = np.argpartition(weights, -16, axis=None)[-16:]
            elif weights.shape == (1, 512):
                max_indices = np.argpartition(weights, -20, axis=None)[-20:]
            elif weights.shape == (1, 1024):
                max_indices = np.argpartition(weights, -24, axis=None)[-24:]
            result = np.zeros_like(weights)
            result.flat[max_indices] = weights.flat[max_indices]
            max_sum = result.sum()
            if max_sum > 0:
                weights = result / max_sum
            ###################################################
...
def get_params():
    params = {
        'weight': 'weights/MAP5094,MAP509559.92.pt',
        'cfg': 'cfg/training/XXXXXXXX.yaml',
        'device': 'cuda:0',
        'method': 'GradCAM',  #
        'layer': 'model.model[XXXX]',
...
if __name__ == '__main__':
    ...
    model('mydata/images/train/10.jpg', 'result/111')
