...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes, multi_level):
        ...
        if self.multi_level:
            self.low_layers = ClassifierModule_GAP(512, [6, 8, 10], [6, 8, 10], num_classes)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=4)
        if self.multi_level:
            self.layer5 = ClassifierModule(1024, [6, 8, 10], [6, 8, 10], num_classes)
        self.layer6 = ClassifierModule_GAP(2048, [6, 8, 10], [6, 8, 10], num_classes)
...
def get_deeplab_v3(num_classes=2, multi_level=True):
